...
Bundled documentation notes are created from files under `docs/bundled-plugin-authoring/` whenever the workspace notes database is bootstrapped (including **headless API** / `npm run start:api` after `NODEX_PROJECT_ROOT` is set). Override the directory with `NODEX_BUNDLED_DOCS_DIR` if needed. The same manifest seeds a **User guide** (application usage) alongside these **Plugin authoring** pages.
