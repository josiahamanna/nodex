...
const VISIBILITY_HINT: Record<ResourceVisibility, string> = {
  public: "Every space member can read and write (subject to their space role).",
  private: "Only you (the creator) can access. Space owners and org admins bypass.",
  shared: "Specific members you add get access. Everyone else in the space is excluded.",
};
...
export type WorkspaceSharePanelProps = {
  workspaceId: string;
  spaceId: string | null;
...
export function WorkspaceSharePanel({
  ...
  initialVisibility,
  canManage,
}: WorkspaceSharePanelProps): React.ReactElement {
  const [visibility, setVisibility] =
    React.useState<ResourceVisibility>(initialVisibility);
  const [shares, setShares] = React.useState<WorkspaceShareRow[]>([]);
  ...
  const refresh = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const rows = await listWorkspaceShares(workspaceId);
      setShares(rows);
      if (spaceId) {
  ...
  async function handleVisibilityChange(next: ResourceVisibility): Promise<void> {
    setError(null);
    try {
      await setWorkspaceVisibility({ workspaceId, visibility: next });
      setVisibility(next);
      if (next !== "shared") {
        setShares([]);
      }
  ...
  async function handleAdd(e: React.FormEvent): Promise<void> {
    ...
    setError(null);
    try {
      await addWorkspaceShare({ workspaceId, userId: addUserId, role: addRole });
      await refresh();
    } catch (err) {
  ...
  async function handleRole(userId: string, role: ShareRole): Promise<void> {
    setError(null);
    try {
      await updateWorkspaceShareRole({ workspaceId, userId, role });
      await refresh();
    } catch (err) {
  ...
  async function handleRemove(userId: string): Promise<void> {
    setError(null);
    try {
      await removeWorkspaceShare({ workspaceId, userId });
      await refresh();
    } catch (err) {
    ...
    <div className="flex flex-col gap-4">
      <section className={card}>
        ...
        {canManage ? (
          ...
          >
            <option value="public">Public — every space member</option>
      ...
      {visibility === "shared" ? (
        <section className={card}>
          ...
          {!loading && shares.length === 0 ? (
            <p className={muted}>
              No explicit shares yet. Add members below to grant access.
            </p>
          ) : null}
          <ul>
            {shares.map((s) => (
      ...
      <p className={muted}>
        Creator, space owners, and org admins always have full access regardless of visibility.
      </p>
