...
# and matches the default `npm run deploy` layout: nodex-api + nodex-web-blue + nodex-gateway.
#
...
docker compose up -d --no-build --remove-orphans nodex-api nodex-web-blue nodex-gateway
