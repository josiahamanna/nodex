...
- **Notes Explorer:** [`useRegisterNotesExplorerPlugin`](../../src/renderer/shell/first-party/plugins/notes-explorer/useRegisterNotesExplorerPlugin.ts) — sidebar [`NotesExplorerPanelView`](../../src/renderer/shell/first-party/plugins/notes-explorer/NotesExplorerPanelView.tsx) renders **[`WpnExplorerPanelView`](../../src/renderer/shell/first-party/plugins/notes-explorer/WpnExplorerPanelView.tsx)** (workspace → project → notes via **`window.Nodex.wpn*`**). The legacy [`NotesSidebarPanel`](../../src/renderer/components/NotesSidebarPanel.tsx) tree remains for non-shell / older layouts.
