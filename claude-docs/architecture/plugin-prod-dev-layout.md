...
- **Bundled read-only plugins:** Trees shipped with the app (Forge `extraResource` → `Resources/system`, `Resources/user`, optional `Resources/core`). Loaded first by `resolveBundledReadonlyPluginRoots` in main and registered as `bundledCoreRoots` in `PluginLoader` (system before user before core).
...
- **Seeded sources:** On first run, sample plugins are copied to `userData/plugins/sources/<name>/` from repo `plugins/user/<name>/` (fallback `plugins/<name>/`, then `plugin-sources/<name>/`) — see `seed-user-plugins.ts`.
