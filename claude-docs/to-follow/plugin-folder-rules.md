...
1. **Bundled read-only plugins** — Repo dev: `plugins/system/` (e.g. `code`), `plugins/user/` (samples + media), optional `plugins/core/`. Packaged: matching `Resources/<basename>` from Forge `extraResource` (`system`, `user`, optional `core`). Resolved by `resolveBundledReadonlyPluginRoots` in main.
...
The repo folders under `plugins/user/` (e.g. `markdown`, `tiptap`) are **samples** until copied into `userData/plugins` (or imported as a zip).
